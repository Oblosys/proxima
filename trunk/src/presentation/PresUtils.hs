...
xyFromPath :: PathPres -> Presentation -> (Int,Int, Bool)
xyFromPath path pres = xyFromPathPres 0 0 path pres

pathFromXY :: (Int,Int,Bool) -> Presentation -> PathPres
...
diffPres :: Presentation -> Presentation -> DiffTree
...
getPres :: IDP -> Presentation -> Maybe Presentation
getPres idp pres = 
  if idp == idP pres 
  then Just pres
  else case pres of
...
deleteInsertedTokens :: InsertedTokenList -> Presentation -> Presentation
...
normalizeRow :: [Presentation] -> [Presentation] -- not fixed for refs
normalizeRow []                      = []
...
normalizeCol :: IDP -> Int -> Int -> [Presentation] -> [Presentation] -> Presentation
...
stripPres :: Presentation -> Presentation
...
--stripRow :: [Presentation] -> [Presentation]
...
-- this only works for simple column of rows with strings
presFromString :: String -> Presentation
...
mouseDownDocPres :: [Int] -> Presentation -> Maybe UpdateDoc
mouseDownDocPres = mouseDownDocPres' Nothing

mouseDownDocPres' :: Maybe UpdateDoc -> [Int] -> Presentation -> Maybe UpdateDoc
...
popupMenuItemsPres :: [Int] -> Presentation -> [PopupMenuItem]
popupMenuItemsPres = popupMenuItemsPres' []

popupMenuItemsPres' :: [PopupMenuItem] -> [Int] -> Presentation -> [PopupMenuItem]
