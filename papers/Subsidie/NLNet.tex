...
\bc
Abstract: explain the whole project in short: (add longer descriptions as attachement, if available)
"
In line with the Web 2.0 trend, an increasing number of web-sites offer visitors the possibility to modify and add content. However, the editors provided are either basic text editors, or custom-made editors that are difficult to build.

A generic editor can be used to create powerful editors with little effort, but has the problem that it needs to be installed. 

Hence, we propose a server-based generic editor. Wysiwyg editors can be created for web-pages, but no installation. Instead client runs a simple script that draws a rendering, and sends edit events to a server.


(A longer project description can be found in the attached project plan)
"
...
"
The NLnet contribution will be used to realize the web-based client-server version of Proxima. Furthermore, a number of novel editors will be instantiated to 
"

Compare your own project with existing projects; what's new and original?
"
The project can be compared with two kinds of existing project: projects about web-based editing and projects about generic editing.

Concerning the web-based editors, On one hand web based editors, such as Wikipedia, Yahoo! Pipes, Google docs. Fixed format

hand XML editors, or generic systems such as Citrus. Not web-based. require heavy installation. 

Because of Proxima's unique layered architecture, it is straightforward to create a web-based client-server version. 
"
\ec
...
\todo{doaitse: wiki}
\todo{what about network problems? googledocs can't handle them}
...
Our short-term goal is to showcase the potential of the technology behind Proxima in order to attract the attention of possible contributors. Proxima is an open-ended system: new object types and presentations can be added to enable the creation of increasingly powerful document types. The hope is that eventually a vibrant community will contribute to a shared repository of resources and push the development of Proxima beyond what a small academia-based group can hope to achieve by itself. \todo{mention that implemented web-page editors will provide good showcase?}
...
An important aspect of Proxima is the fact that a presentation may contain computed values and structures. This is apparent in Figure~\ref{fig:heliumEditor} in the line before the declaration of \p{c}. The value \p{16} is computed automatically, and changes when parameter \p{3} is edited or when the function \p{f} is modified.
...
mouse events:
<input type="checkbox" name="http://javascript.internet.com/page-details/mouse-coordinates.html
" value="">http://unixpapa.com/js/testmouse.html
\ec