...
An editor for a subset of the functional language Helium~\cite{heeren03helium} has been implemented with Proxima. Figure~\ref{fig:heliumEditor} contains a screenshot of this editor. \todo{mention Bayes editor?}
...
The positions at which the document, the rendering, and the intermediate data structures reside are called {\em data levels}. Between each pair of levels is a {\em layer}, which is a component that maintains the mappings between the levels. Figure~\ref{fig:levelsAndLayers} schematically shows the levels and layers of Proxima. Only two data levels are visible to each layer: a higher and a lower level.\todo{name individual components and show sheets + positions at which we can edit}
