...
Now, we can capture the \p{Step} constructor and the lambda expression with the function \p{liftStep}:
...
In order to remove the \p{Comp} constructor, we observe that the number of \p{Comp} applications is one less than the number of steps, and corresponds to the number of function compositions on the first line of \p{lift}. Hence, we can define a composition function \p{lcomp}, which adds a \p{Comp} constructor to its first argument:

\begin{small} % lift5
\begin{verbatim}
lcomp f g = (\n s -> Comp $ f n s) . g

lift simple = lfix $ step1 `lcomp` step2
...
{\tt lift layer = } \= {\tt lfix    liftStep (layerFn$_1$ layer)} \\
                    \>       \dots \\
                    \> {\tt `lcomp` liftStep (layerFn$_n$ layer)}\\
\end{tabbing}%$
\end{small}
...
The explicit mutual recursion in the local functions is removed by passing the next step as a parameter, and rewriting the whole function as a fixed point.
...
\begin{verbatim}
combine = cfix $ step1 . combineStepUp
 where step1 next (Comp upr) (Comp lwr) = Comp $ 
          combineStepDown next upr lwr  
\end{verbatim}%$
\end{small}

The \p{Comp} constructors can be handled by a slightly modified version of function composition:
...
%																
\subsection{A type class for unwrapping} \label{sect:decompose}

A minor problem with the version of \p{editLoop} from the previous section is the varying amount of \p{Comp} constructors, which depends on the number of layer functions. The problem is minor, since typically the pattern matches appear only in a single function where all layers are combined. Furthermore, the number of layer functions is not likely to change often during the development of a system. However, it is possible to define a type class that takes care of the pattern matching.

\begin{small}
\begin{verbatim}
class Decomp comp t decomp where 
...
A drawback of using this class is that all types must receive explicit signatures, because otherwise the type checker will require an instance that is too general and cannot be inferred. A functional dependency \p{comp t -> decomp} seems appropriate to alleviate this problem, but does not work with the two instances. The reason lies with the overlapping instances giving rise to conflicting types \p{decomp}. For example, for \p{t1 :.: t2} and \p{t}, the first instance declares \p{decomp} as \p{(t1 :.: t2) t}, whereas the second instance gives \p{t1 (t2 t)}.

A solution to this problem is to declare an explicit base \p{TNil} for composed types. Thus, instead of \p{t1 :.: t2} we get \p{TNil :.: t1 :.: t2}. Because of the partial parameterization, \p{TNil} is a newtype:
...
Now, instead of the overlapping instance \p{Decomp f t (f t)}, we can define an instance for \p{TNil} that does not overlap:
...
Unfortunately, this introduces yet another newtype constructor. Worse still, there is no metacombinator that can be modified to handle this constructor, so a special nil step needs to be added to both \p{lift} and \p{combine}. Because of this, and the fact that the functional dependency adds extra complexity, we choose to keep the simpler representation and provide explicit type signatures.
