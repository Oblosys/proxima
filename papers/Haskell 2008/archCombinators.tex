...
Figure~\ref{simpleeditprocess} depicts the editing process for our simple editor. The editor keeps track of a document (\p{doc}), which is mapped onto a presentation (\p{pres}). The presentation process is split into $n$ components: $\mathit{present}_1 \dots \mathit{present}_{n}$. At the bottom of the figure, the presentation is shown to a user, who provides an edit gesture (\p{gest}) in response. The edit gesture is then mapped onto a document update (\p{upd}) by $\mathit{interpret}_{n} \dots \mathit{interpret}_1$, after which it is performed on the document.
...
Note that the field names in the record (\p{present}, \p{interpret}) are also the names of the field selection functions. Thus, for a layer \p{layer1 :: Simple} we denote the presentation function by \p{present layer1}. 
...
We give an example edit loop that explicitly connects three layers: \p{layer1}, \p{layer2}, and \p{layer3} of type \p{Simple}. The data flow between the layer functions is shown in Figure~\ref{explicit}. The corresponding Haskell code for the edit loop is:
...
Example of nested pairs are \p{((e$_1$,e$_2$),e$_3$)}, and \p{((e$_1$,e$_2$),(e$_3$, e$_4$))}. Although not enforced by the combinators, we only use left-associatively nested products in this section: 
...
\bc and \p{((e$_1$,e$_2$),(e$_3$,e$_4$))} are valid examples.\ec
...
\bc Because nested pairs with different structure are of different type, we use the combinators in this section only in a right-associative way, so all products are of the form \p{(e$_1$,(e$_2$,(e$_3$, (\dots, e$_n$)\dots)))}. 
\ec
...
\bl
\o Want to get rid of layer and step specific stuff.
\o Layer is a problem, needs to add/remove constructor.
\o use a different rep.
\o steps.
\o composing.
\o fix the whole thing.
\o need at least one newtype, but since type syns can't be partially parametrized.., all involved types need newtypes
\o as we will see, combinators can take care of adding/removing the constructors.
\el
\begin{small}
\begin{verbatim}
