...
\todo{introduce proxima}
for proxima
generic presentation-oriented editor for structured docs.
proxima supports editing on the document structure but also on its presentation. Has a layered arch, in which layers sometimes have state that does not belong on other levels
...
Although type errors become somewhat more complicated because of the overloaded types, the type class solution turns out to be quite useful. The monadic version has been succesfully tested in the Proxima generic editor.
...
\todo{algemene beschrijving van data flow}
...
Apart from the switched directions in \p{combine}, the definitions for the Proxima architecture are virtually the same as for \p{Simple}. Because of the similarity, we do not reproduce the entire architecture description here.
