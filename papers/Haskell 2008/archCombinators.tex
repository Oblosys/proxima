...
second is just app. first has some constrs
In order to get rid of the \p{Fix} application, we first rewrite \p{combine} to make the pattern more apparent:
...
If we use \p{ccomp}, and drop the paramters to step1yielding a So we have

\begin{small}%combine6
\begin{verbatim}
combine6 = cfix $ step1 `ccomp` combineStepUp
...
Small problem. Need explicit types, otherwise tries instance that's too general.
functional dependency \p{comp t -> decomp where} solves the problem but does not work with these instances, as the first instance also matches second and gives conflict between the fun dep. A solution is to use a base for the recursion. so  Nil :.: a :.: b. But this will clutter the defs of lift and combine since it will be yet another newtype with a constructor.
...
Because a layer in Proxima may perform IO operations, we use a monadic version of library, which is a straightforward extension of the code in Figure~\ref{metacombinators}. In the monadic version, each let expression of the  form:
...
The rest of the library is defined analoguosly. fix etc. are not affected. 
