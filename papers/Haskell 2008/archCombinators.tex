...
Now, for example, we can encode \p{Doc -> (Pres, Gest -> (Upd, next))} as
\p{(Step Doc Pres :.: Step Gest Upd :.: NilStep) next}. To encode the recursion, we introduce a fixed-point type \p{Fix}:
...
The reason why we have an explicit \p{NilStep} with yet another constructor is that it causes the number of compositions (\p{:.:}) to be the same as the number of steps, which will facilitate the removal of \p{Comp} constructors.
...
In this code, we can identify a composition function that takes a varying number of functions (\p{liftStep lf1} to \p{liftStep lfn}) and return a composition. If we assume a function \p{compose}, that takes a representation of the number of steps (denoted by \p{<N>}) and and neutral element for the composition, we can rewrite \p{genericLift} to:

\begin{small}
\begin{verbatim}
genericLift = \lf1 .. lfn -> 
              lfix (compose <N> id (liftStep lf1) .. (liftStep lfn))
\end{verbatim}%$
\end{small}

Now we need a function that given a function f g  (compose), takes n arguments, applies them to the function and applies lfix to the result.
...
Now we need to get a <N> the result is what we need, it contains many comp steps.  
...
As the reader may have guessed, the last definition is no longer pseudo code, as \p{compose}, \p{app}, and \p{resType} can be defined withe type classes. Thanks to the power of Haskell's class system, it turns out to be possible to define . 


For the variable-argument composition, we declare a class \p{Comp} with a single method \p{compose}, which takes a composition type, and a neutral element, and returns a composition function that takes as many arguments as the composition type has steps.
...
The code function \p{liftStep} is the same as in the previous section, since the phantom-type does not show up in the values. 
...
Similar to genericLift, we can assume a function creates a composition of n combineStepUp/Downs, based on the type. .
...
\end{verbatim}
\end{small}\todo{maybe not show Up instance?}
\bc
instance (Combine c ct ( (ut -> lt -> ct) ->
                        u ut -> l lt-> c ct) ) =>
...
\ec
There is also an instance for \p{Step Up} but it is simply Down, with Down replaced by up, and the paramaters m r a m  replaced by a m m r. Hence we do not show it here.
...
With the two generic functions defined above, it is no longer necessary to manually define a \p{combine} function.

Just give a type and a fix and it works. main looks like this now:
...
Drawback much complexity. If no type sig, type of combined layers is \p{a} with a whole lot of constraints. Certain type errors will be of the form \p{cannot deduce instance}. although still a lot are readable.
...
\head{Definition of \p{lift} and \p{combine}} \todo{account for monad}
