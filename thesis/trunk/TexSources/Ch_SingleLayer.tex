...
\xpr{
\eqcl{\level''\M}{HL} = \present\H~\eqcl{\level''\H}{HH} ~~~\imp~~~ \eqcl{\present~ \eqcl{\level''\M}{LH}   ~\reuze{LL}~   \level'\L}{CL} = \present\C~\eqcl{\level''\H}{CH} \\
}
...
\begin{Prf}&
	\eqcl{\level''\M}{HL} = \present\H~\eqcl{\level''\H}{HH} \\
\Eqv{reflexivity of $=$}&
 	\present\L~ \eqcl{\level''\M}{LH} = \present\L~\eqcl{\level''\M}{LH} \land
 	\eqcl{\level''\M}{HL} = \present\H~\eqcl{\level''\H}{HH}\\
\Imp{let $m = \level''\M$}&
 	\exists m :\present\L~ \eqcl{\level''\M}{LH} = \present\L~\eqcl{m}{LH} \land
 	\eqcl{m}{HL} = \present\H~\eqcl{\level''\H}{HH}\\
\Eqv{\law{$\reuz$-Valid}}&
 	\exists m :\eqcl{\present\L~ \eqcl{\level''\M}{LH}   ~\reuze{LL}~   \level'\L}{LL} = \present\L~\eqcl{m}{LH} \land
 	\eqcl{m}{HL} = \present\H~\eqcl{\level''\H}{HH}\\
\Eqv{\law{$\present\C$-Def}}&
...
We need an extra requirement in order for \law{Doc-Inert} to hold.

From \law{InterPresent}, we know that if $\level'\L$ is a presentation of $\level\H$, then $\level\H$ is in the interpretation of $\level'\L$. Thus, there exists a middle level . However, the existence of such a middle level does not guarantee that the middle level computed by $Comp$ is . 
...
For the wildcard types from Section~\ref{}, \law{Orthogonal} holds ...
...
Similar to the \law{Doc-Inert} requirement, we need an extra condition for \law{Pres-Inert} to hold.
...
Another one is \law{Compatibility} see Section~\ref{}. proof gets nasty.
...
% or say it with Hoare?
%\xprlab{\hoare{\eqcl{\level'\L}{L} = \present~\eqcl{h}{H} \land \level_{n+1} = x} {Comp_n} { \level'\L = \level''\L  \land %\level_{n+1} = x}}	
...
\xpr{ \eqcl{\level'\L}{CL} = \present\C~\eqcl{h}{CH} ~\imp~ \wp{Comp_n}{\level'\L = \level''\L}}
...
beacuse of bla, we also have\note{maybe add it to pre}
...
because easy to see that $Comp_n$ only affects levels $n \dots 0$.

Finally bt \law{\fn{wp}-And}, we get the whole thing:
...
rewritten \law{Sane}:
\xpr{
\forall m_0 & : & (\exists  : \eqcl{m}{LH} = \eqcl{m_0}{LH} \land \valid~m) ~\imp~ \exists m_1 : \eqcl{m_1}{LH } = \eqcl{m_0}{LH} \land \eqcl{m_1}{HL} = \present\H (\interpret\H \eqcl{m_0}{HL})
}
