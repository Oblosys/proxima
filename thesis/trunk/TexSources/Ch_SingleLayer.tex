...
In order to relate the data levels surrounding one layer, we use two invariants: the {\em presentation invariant} and the {\em translation invariant}. The presentation invariant is satisfied if and only if the lower level is a correct presentation of the higher level. It is expressed using an abstract function $Present$:

\begin{math}
Present :: Level_{High} \rightarrow Level_{Low} \\
\end{math}

Similarly, the translation invariant is satisfied if and only if the higher level is a correct translation of the lower level. The translation invariant is expressed with a function $Translate$:

\begin{math}
Translate ::  Level_{Low} \rightarrow Level_{High} \\
\end{math}

** they are not functions but relations
\note{not functions but relations because of extra state}
...
** use Edit to bla and trans bla because one layer. next chapter connect everything:

Expressed as equations, the invariants \note{are these things really invariants?} are:
...
Figure~\ref{nodeMapping} shows an example of such a mapping between two levels that satisfy the presentation invariant. The dotted arrows denote on which lower level nodes a higher level node is mapped. To reduce the number of arrows in the figure, the lower level nodes are grouped.
...
First we take a closer look at the downward, or presentation, direction. $Present$ maps each element of the higher level onto zero or more lower level elements, forming zero or more subtrees with holes. \note{is there a name for "tree with holes?"} Figure~\ref{elementPresentation} shows the presentation of one node ($E_H$) in the higher level. $E_H$ is an element in the higher data level that has $n$ children ($C_{H,0}$, \dots, $C_{H,n-1}$). The presentation of $E_H$ is a number of trees (although usually just one) that may consist of several nodes. In the figure, the first tree is shown in more detail. It consists of $m$ nodes has root at $E_{L,0}$. 
...
\begin{center}
\begin{center}
\epsfig{file=pics/eps/presentParenSum.eps, width=4in}
%
%  ( )       Tk   ( 1+2 )
%   +      Tk ...
%  1 2       .....  
...
***
Figure~\ref{presentExample} provides a more concrete example of a presentation. A parenthesized sum in the enriched document is presented as a list of tokens on the presentation level. The unlabeled nodes group o is concat. tuples are is extra state (line breaks, spaces)Section~\ref{layout in arch} so final is "\verb|( 1+2 )|" 
...
A consequence of the difference between the presentation and the translation mappings is that even when a lower level element depends on several higher level elements, only one is responsible for its presentation. \note{where does this become important?} \note{example: leaf Color Int -> colored int. When selecting, either lead or int,  Color affects but is not responsible. can be accessed by special edit ops though.}

\bigskip {\bf Duplicates in presentation}

\note{duplicates in translation?}
...
During editing, data levels are updated as a result of user interaction. When a level changes, the adjacent level needs to be updated to establish the appropriate invariant. We specify two mapping functions  ${\tt present}$ and ${\tt translate}$ that compute new levels. On a higher level update, ${\tt present}$ computes a lower level that establishes the presentation invariant. Similarly, on a lower level update, a new higher level is computed by the ${\tt translate}$ mapping, establishing the translation invariant. 
