...
\level'\L = \present~h \imp \level''\L = \level'\L			& \jstf{\sc Pres-Inert}\\
}
...
\newcommand{\fn}[1]{{\ensuremath{\sf #1}}}
...
\head{Hoare} \note{say this somewhere else}

\xprlab{
\assert{P} S \assert{Q} ~\eqv~ P \imp wp(S,Q)
}{\fn{wp}-Char}
...
or with Hoare triples \note{change compute to $\gts$ notation}
\note{say something about termination}


\head{Postcondition}

$\assert{True} {Comp} \assert{ \level''\L = \present~\level''\H }$,
...
\begin{proof} 
$\assert{ \level'\L = \present~\level\H } {Comp} \assert{ \level''\H = \level\H }$, or equivalently, due to \law{\fn{wp}-Char}:
...
%This leaves us to prove: $\level'\L = \present~\level\H \imp \level\H = \interpret~\level'\L $, which corresponds to %{\sc InterPresent}.

%We can prove 
%$\assert{ \level'\L = \present~\level\H } {Compute} \assert{ \level''\H = \level\H }$
%
%using Hoare logic:
%
%\prf{ 
%\assert{ \level'\L = \present~\level\H }\\
%\qquad \level''\H \gets \interpret~\level'\L \\
%\assert{ \level\H = \level''\H  }\\
%\qquad \level''\L \gets \present~\level''\H \\
%\assert{ \level\H = \level''\H  }
%}
%
%This leaves us to prove: $\level'\L = \present~\level\H \imp \level\H = \interpret~\level'\L $, which corresponds to %{\sc InterPresent}.
...
% Old Hoare proof
%
%$\assert{\level'\L = \present~h} Compute \assert{ \level''\H = \level\H }$
%
%Using assignment rule twice, we prove:
%
%$\assert{\level'\L = \present~(\interpret~\level'\L) } Compute \assert{ \level''\H = \level\H }$
%
%\begin{proof}
%\prf{
%\assert{ \level'\L = \present~(\interpret~\level'\L) }\\
%\qquad \level''\H \gets \interpret~\level'\L \\
%\assert{ \level'\L = \present~\level''\H }\\
%\qquad \level''\L \gets \present~\level''\H \\
%\assert{ \level'\L = \level''\L  }
%}
%
%And:
%
%$(\level'\L = \present~h) ~\imp~ \level'\L = \present~(\interpret~\level'\L)$
%
...
%\Ffr{assumption}\\
%\level'\L = \present~(\interpret~(\present~h_0))\\
%\Eqv{\sc InterPresent}\\
%\level'\L = \present~h_0 \}\\
%\Ffr{assumption}\\
%True
...
\xprlab{
Up ~~\is~~		& \level''\H = \interpret~ \eqcl{\level'\L}{L}  ~\reuze{H}~   \level\H}{$Up$-Def}
\xprlab{
Dwn ~~\is~~		& \level''\L = \present~ \eqcl{\level''\H}{H}   ~\reuze{L}~   \level'\L}{$Dwn$-Def}
...
$\assert{\eqcl{\level'\L}{L} = present~\eqcl{\level\H}{H}} Compute \assert{ \level\H = \level''\H }$
...
\Eqv{bla char}\\
\level\H \in interpret~\eqcl{\level'\L}{L} \\
\Imp{{\sc $\reuz$-Idem} ?+ result of $interpret$ is eq. class?}\\
...
\wp{Up}{ \wp{\level''\L \gets \present~ \eqcl{\level''\H}{H}   ~\reuze{L}~   \level'\L}{ \level\H = \level''\H }}\\
	\Eqv{{\sc \fn{wp}-$\smi$}}\\
\wp{Up \semi \level''\L \gets \present~ \eqcl{\level''\H}{H}   ~\reuze{L}~   \level'\L}{ \level\H = \level''\H }\\
	\Eqv{\law{$Dwn$-Def}}\\
...
%\begin{proof}
%
%Assignment rule gives us:
%\prf{
%\assert{ \level\H = \interpret~ \eqcl{\level'\L}{L}  ~\reuze{H}~   \level\H }\\
%\qquad \level''\H \gets \interpret~ \eqcl{\level'\L}{L}  ~\reuze{H}~   \level\H\\
%\assert{ \level\H = \level''\H }\\
%\qquad \level''\L \gets \present~ \eqcl{\level''\H}{H}   ~\reuze{L}~   \level'\L\\
%\assert{ \level\H = \level''\H }
%}
%
%From which we can prove {\sc Doc-Inert} by weakening the precondition, if we have:
%
%$\eqcl{\level'\L}{L} = present~\eqcl{\level\H}{H} ~\imp~ \level\H = \interpret~ \eqcl{\level'\L}{L}  ~\reuze{H}~ %  \level\H\\$
%
%\prf{
%\eqcl{\level'\L}{L} = present~\eqcl{\level\H}{H}\\
%\Eqv{\sc InterPresent}\\
...
\prf{
\assert{\exists h : \eqcl{\level'\L}{L} = \present~\eqcl{h}{H}} Compute \assert{ \level'\L = \level''\L}\\
}

\prf{
\assert{ \level'\L = \present~ \eqcl{\interpret~ \eqcl{\level'\L}{L}  ~\reuze{H}~   \level\H}{H}   ~\reuze{L}~   \level'\L }\\
\qquad \level''\H \gets \interpret~ \eqcl{\level'\L}{L}  ~\reuze{H}~   \level\H\\
\assert{ \level'\L = \present~ \eqcl{\level''\H}{H}   ~\reuze{L}~   \level'\L }\\
\qquad \level''\L \gets \present~ \eqcl{\level''\H}{H}   ~\reuze{L}~   \level'\L\\
\assert{ \level'\L = \level''\L  }
}
...
$\eqcl{\level'\L}{L} = \present~\eqcl{h}{H} ~~\imp~~ \level'\L = \present~ \eqcl{\interpret~ \eqcl{\level'\L}{L}  ~\reuze{H}~   \level\H}{H}   ~\reuze{L}~   \level'\L $

Assume $\eqcl{\level'\L}{L} = \present~\eqcl{h}{H}$, then: \note{existential quant. for $h$?}
