...
In this chapter we use the functional programming language Haskell to give a domain specific embedded language (DSEL) for describing the layered architecture of Proxima that is described in Chapter~\ref{chap:proxArch} and specified in the previous Chapter. ***Layered archs in general, not only prox. Also used for a ...??
...
In this section, we introduce a simple layered architecture for an editor that is used in the next sections to illustrate the architecture description methods. The system that is described by the architecture is an editor in which the user can edit a presentation of a document. Although the architecture of the editor is simple, it still contains the typical features of the layered architectures** discussed in Chapter~\ref{chap:formalSpec} \note{is chapter now}. 
*****
\bc
...
% \save"2,2"."3,2"*!=<6.105em,3.2ex>[F.]\frm{}\restore 
