...
smaller font for code fragments
...
In this chapter we use the functional programming language Haskell to give a domain specific embedded language (DSEL) for describing the layered architecture of Proxima, which was described in Chapter~\ref{chap:proxArch} and specified in the previous Chapter. The method was designed for describing ***Layered archs in general, not only prox. Also used for a ...?? \todo{afmaken Jurre Laven}
...
In this chapter we give four implementation models for layered architectures. First, we introduce a simple example layered editor architecture and explore how its main components can be modeled in Haskell. Then we proceed to connect the components. In Section~\ref{sectsimple} the connection is straightforward, with little abstraction. This is used in Section~\ref{sectncp} as a basis to develop a more abstract combinator implementation that uses nested cartesian products. In Section~\ref{sectdpp}, we present another set of combinators, which employ a form of state hiding to improve on the previous set. Section~\ref{sectlib} develops a small generic library for building the architecture-specific combinators of Section~\ref{sectdpp}. Finally, in Section~\ref{sectproxima}, we show how the architecture combinators are used to describe (and implement) the architecture of  the Proxima editor.%Section~\ref{secthaskellconclusions} presents the conclusions.
...
We introduce a simple layered architecture for an editor, which is used in the next sections to explain the architecture description methods. The system that is described by the architecture is an editor in which the user can edit a presentation of a document. Although the architecture is simple, it contains the essential features of the Proxima architecture discussed in Chapter~\ref{chap:proxArch}. Section~\ref{sect:proxima} \todo{}
...
\o doc is presented by \texttt{present}, shown to the user
\o interpreted.
\el
...
Both pres as interpr are layered functions. 

example of hor is values of the levels, so vertical are just edit ops. This is what is done in Proxima (see Section~\ref{sect:proxima})
...
\end{scriptsize}
\end{center}\caption{Data flow in layer. (draft)}\label{wrapped} 
\end{center}
\end{small}
\end{figure}
...
\end{scriptsize}
\end{center}\caption{ Horizontal parameters in nested cartesian products. (draft)}\label{ncp} 
\end{center}
\end{small}
\end{figure}
...
\end{tiny}
\end{center}\caption{\texttt{composeDown} and \texttt{composeUp} (draft)} \label{composeDownUp} 
\end{center}
\end{small}
\end{figure}
...
\head {\bf Combining layers:}  Using \texttt{composeUp} and \texttt{composeDown}, we can define a combinator to combine \texttt{Simple} layers. From now on, we use the type \texttt{Simple} instead of \texttt{Simple'} because the matching horizontal parameter types in \texttt{Simple} make the connection easier. 
...
\todo{Johan: "enforce that the horizontal parameters are tuples (ook niet helemaal)"}
...
\head{Simple editor:}  The main editor loop from the previous section now reads:
...
\head{Lift:} The combinator \texttt{lift} takes a layer of type \texttt{Simple} and returns a \texttt{Layer} value:
...
\head{Combine:} With \texttt{lift} we can instantiate layer values of type \texttt{Layer doc pres gest upd}. To combine layers, we define a combinator \texttt{combine} with the following type:
...
\head{Simple editor:} The edit loop of the simple editor no longer has a reference to the horizontal parameters. Furthermore, the combined layer is called \texttt{presentStep} instead of \texttt{layers}, to reflect that it is the present step of the computation.
...
\todo{Johan: (over \texttt{lift simple} hieronder) "hier introduceer je een fixed point. Misschien iet meer woorden aan besteden?"} 
% ref: Augustson ICFP??
...
We resolve the ambiguity by adding a functional dependency~\cite{jones00fundep}~ \texttt{step -> arg res} between \texttt{step} and \texttt{arg} and \texttt{res} to the class declaration.
...
\head{Simple editor:} As an example application of the library, we give the definitions that are required for implementing the simple editor architecture. Recall that the data type containing the layer functions is:
...
\bl
\o Introduce monadic combinators
\o picture of proxima layer + explanation
\o maybe some code fragments
\el

\bl
\o Monadic. simple monad part of the type, results are m.. and let = is replaced by \verb|<-| in a monad comprehension
\o does not matter much, so not shown here
\el
**********
