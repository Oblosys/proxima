...
%al results with example editors built with the prototype seem to suggest the validity of our approach. \todo{zin?}
...
%\item[valid document:] A type-correct document, mainly used for XML documents.
%\item[valid resentation:] Presentation that is pres. of a document 
...
An editor that knows about the structure of the edited document can interesting\todo{Doaitse: ``a lot of offer document-type specific'', maar a lot of is niet ok.} functionality. We list several potential advantages of generic structure editors. The first two advantages stem from the genericity of the editor, whereas the rest are mainly about the structural (document-oriented) abilities.
...
In Chapter~\ref{chap:presenting}, we discuss the {\Xprez} presentation formalism for the Proxima editor. {\Xprez} is a declarative ... . reqs. Important for editor.
\fromHere
