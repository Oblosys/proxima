...
As an example, consider an update on the presentation of the $Decl~\verb|"f"|~(Plus~1~2)$ declaration from the example above:  $\verb|f = ...| \leadsto \verb|g = ...|$. The new higher level should have left-hand side \verb|"g"| and reuse the sum. This can be specified by requiring that from the equivalence class of declarations with left-hand side \verb|"g"|, we select the element that is as close as possible to $Decl~\verb|"f"|~(Plus~1~2)$. The obvious solution is $Decl~\verb|"g"|~(Plus~1~2)$.
...
Similar to \law{Doc-Inert}, we cannot guarantee \law{Pres-Inert} without assuming extra conditions. It turns out that we can prove \law{Pres-Inert} if we assume \law{$\present$-Match} together with a reversed \law{Orthogonal}, in which the $HL$ and $LH$ relations are swapped:
...
{$\present$-Match} and {$\interpret$-Match} conditions on the components. Furthermore, for \law{Doc-Inert} and \law{Pres-Inert} requirements, we need to assume the conditions \law{Orthogonal} and \law{Orthogonal-R}. 

Because an editor consisting of $n$ layers is constructed by taking the top layer and repeatedly putting layers underneath, we need to prove these conditions for each of the $n-1$ combinations.
...
We give two examples to illustrate the behavior of $\Delta$. For the first example, consider the presentation function $\present~(x,y) = (x,x,x,y,y)$. If a user updates the second element of the presentation tuple (eg. $(1,1,1,5,5)\leadsto(1,2,1,5,5)$), we compute 
...
\el
\bc
In this chapter, we have provided a specification of the $\interpret$ mapping, given a presentation mapping $\present$. Together with the this constitutes a specification of an editor.

\bl
\o specification of a presentation-oriented layered editor with extra state in either direction
\el
...
\o \cite{muhu04inv} a small functional language is. not clear how to scale, plus extra state only in one direction.
\el


The combination of extra state and duplications in the presentation allow 
Although it is rather easy to find pathological cases that cannot be specified with our model, . by the specification
...
An important area of future research lies in the development of a presentation language that is powerful enough for specifying the use cases of Chapter~\ref{} but which . Because an efficient inverse cannot always be computed automatically, complex parts of the presentation could be annotated, while for simpler presentations. Until such a language has been developed, the burden of guaranteeing correctness of an editor lies with the implementor.
