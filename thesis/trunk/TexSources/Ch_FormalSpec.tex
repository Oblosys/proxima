...
Note that in constrast to the architecture description in Chapter~\ref{sect:proxArch}, $\present$ and $\interpret$ in the specification are functions between values rather than edit operations.

\todo{Unclear: Say some things on totality of \interpret (is it easy to make it total?)??}
...
\todo{same as before, but now also specify }
...
\todo{What about \law{InterPresent}?}
