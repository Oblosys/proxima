...
The problem with the \law{Imprecise} requirement is that the duplicates that were not edited influence the result at the cost of the edited duplicate. We tackle the problem by weakening the \law{Imprecise} requirement. Rather than requiring the entire final presentation to be ``close to'' the result of the user update, we only require closeness for those parts of the presentation that were changed.
\note{mention already that a weaker form is still needed?}

In order to make explicit which parts of $\level\L$ have changed, we introduce a new operator:
...
The new \law{Imprecise} requirement only requires closeness on the edited part of the presentation:
...
To prevent \law{Pres-Inert} from suggesting updates on hidden parts of the document, we weaken the requirement in a similar way as \law{Imprecise}; both the pre- and the postcondition are changed to refer only to the updated parts of the lower level:
...
\note{\law{Pres-Inert} now seems cover some of \law{Imprecise}'s cases}.
