...
In the presence of extra state, $\Present$ is an actual \note{} relation, which cannot directly be represented by a function. Therefore, we introduce an equivalence class representation for extra state, which allows us to represent $\Present$ by a function between equivalence classes.

\note{and the types}
Because we make only few assumptions on the presentation mapping, the specification gives requirements for $\interpret$, rather than an exact specification of how the inverse can be computed for a specific $\present$. The burden of defining an $\interpret$ that meets the requirements lies with the editor designer.
...
We develop the specification of the layered editor by starting with a simplified editor without layers, and then step by step adding extra functionality. Section~\ref{sect:singleSimple} specifies a simple editor that consists of a single layer and does not support extra state. In Section~\ref{sect:singleExtra}, the specification is extended with a general model of extra state. Section~\ref{sect:wildcardEq} provides a more concrete specification of extra state for tree data structures. The last step is the addition of layers to the specification in Section~\ref{sect:compositeExtra}. Finally, Section~\ref{sect:duplicates} informally discusses how the specification may be adapted to handle duplications in the presentation.
...
The wildcard for type $T$ represents any possible value of type $T$. Thus, the value $\texttt{*}_{Bool}$ represents any value from the set $\set{{\tt True}, {\tt False}}$, and the value $\texttt{*}_{Int}$ represents any integer. An example shows how wildcards can be used to model extra state in a data type.
...
For a type $T$, a type $T^*$ declares which parts are extra state. Note that for a single type $T$ several types $T^*$ are possible: whether or not a value is extra state is not determined by its type, but rather by the $T^*$ declarations in which  the value appears as a child. Hence, in the example, a boolean in a ${\tt Bin}$ node is extra state, whereas a boolean in a ${\tt Leaf}$ is not. 
...
This case does not occur, since the assumption $y \in \para{x\st}$ implies $\p{C} \equ \p{C}'$:
...
\p{C}_{id}~x^*_0 \dots x^*_n ~\reuzestar~ y = \p{C}_{id}~x'_0 \dots x'_n\\
~\textbf{where}~x'_i = \left\{
\begin{array}{lll}
x^*_i ~\reuzestar~ y	& \textbf{if}~x^*_i \neq \texttt{*}_{t}\\
y_i  				& \textbf{if}~\fn{lookup}~id~y = \p{C}~y_0 \dots y_n \\
y_j  				& \textbf{if}~\fn{lookup}~id~y = \p{C}'~y_0 \dots y_n \land \p{C} \neq \p{C}' \\
...
Before we show the compositions for $\present\C$ and $\interpret\C$, we first take a look at what the composition looks like if we view the mappings as relations $\Present\C$ and $\Interpret\C$ rather than as functions between equivalence classes. The relation view is somewhat simpler because it does not explicitly refer to the various extra state equivalence classes in the composition.

\head{Composite $Present\C$ and $Interpret\C$}

For the specification of a composite interpretation mapping, we split (or decompose) the presentation mapping whereas we compose the interpretation mapping. However, from a formal standpoint there is no distinction between splitting and composing, since both imply that the composite relation is the relational composition of two other relations. Hence, we ignore this distinction for $Present\C$ and $Interpret\C$.
...
We can split $\present\C$ and compose $\interpret\C$, similar to $Present\C$ and $Interpret\C$  with the difference that all relations are now represented by functions between equivalence classes. This gives rise to various equivalence classes, which are explained in more detail below.

Because not every composition of two relations that are functions between equivalence classes yields a relation that is a function between equivalence classes itself, the definitions below need additional restrictions to be valid. We will provide these restrictions in the form of two requirements for the components of $\present\C$ and $\interpret\C$.
...
We assume the existence of $\present\C$, which, for a certain $\Level\M$ (and certain equivalence classes $HH$, $HL$, $LH$, and $LL$), can be decomposed into 
$\present\H  \tp \Eqcl{\Level\H}{HH} \to  \Eqcl{\Level\M}{HL}$  and $\present\L \to \Eqcl{\Level\M}{LH} \rightarrow  \Eqcl{\Level\L}{LL}$, similar to relation $\Present\C$. (Note the reversed order in the types, as was explained in Section~\ref{sect:editProces}.)
...
Because the equivalence classes on the middle level do not match ($\Eqcl{\Level\M}{HL}$ versus $\Eqcl{\Level\M}{LH}$), we cannot simply use function composition for $\present\C$ and $\interpret\C$. However, we can use Kleisli composition on set-valued functions to give a equivalent point-free definition of  the compositions:
...
Before we discuss the restrictions we need for both compositions to be valid, we take a look at the equivalence classes that play a role in a composite layer.

\head{Equivalence classes in a composite layer}
...
From the types of the $\present\C$, $\present\H$ and $\present\L$, we see that there are six equivalence classes involved. Both the upper and lower layer have equivalence relations on their respective upper and lower levels: $HH$, $HL$, $LH$, and $LL$. 
...
$(\present_{n-1} \oo \cdots \oo \interpret_{n-1})$. This corresponds to splitting off an atomic layer at the bottom.
...
Similar to \law{Doc-Inert}, we cannot guarantee \law{Pres-Inert} without assuming extra conditions. It turns out that we can prove \law{Pres-Inert} if we assume \law{$\present$-Match} \bc \todo{not \law{$\interpret$-Match}??} \ec together with a reversed \law{Orthogonal}, in which the $HL$ and $LH$ relations are swapped. However, because \law{Orthogonal} refers to $\reuze{LH}$, its reverse would have to refer to $\reuze{HL}$, which does not exist. The reason for this is because of the association order in $Comp$, reuse in the presentation direction of the higher layer is the result of the combined effect of the $\reuze{LL}$ functions of the components of the higher layer. Thus, the only way to specify a reversed \law{Orthogonal} is as a condition on the composite layer:
...
We do not wish to assume \law{Absorption} for the top-most composition ($\present_1$, $\interpret_1$ and the identity), because it implies that the interpretation extra state of $\present_1$ is absorbed by the presentation extra state of $\id$. As a consequence, the top-most composition would not have interpretation extra state, and therefore neither of the composite layers.


%and thus none of the layers would since all other interpretation extra state is absorbed by presentation extra state.
\toHere
Dit nog even snappen en goed uitleggen.
\fromHere

To allow interpretation extra state at the top-most composite layer, we give a separate proof the case $n=1$ for \law{Pres-Inert}, in which we do not use \law{Absorption}. 

An important consequence of the fact that we do not assume \law{Absorption} on the top-most composite layer is that in the proof of \law{Doc-Inert}, we cannot use \law{Absorption} to imply \law{$\present$-Match}, \law{$\interpret$-Match}, and \law{Orthogonal} for case $n=1$. However, since for $n=1$, all three conditions trivially hold, we do not need to assume \law{Absorption} for this case.
...
\todo{more on nature of composite closeness?}
%\bl
...
\note{split instead of constructed}
...
Edit operations on duplicates most probably result in an invalid presentation, unless a user has managed to consistently edit all occurrences of the duplicate. Hence, to a large extent, the result of an edit operation on a duplicate value is specified by the \law{Imprecise} requirement: 
...
The precondition has been weakened because otherwise the presence of duplicates in the presentation may disable the requirement. The postcondition, on the other hand, is weakened because we only require the updated parts to stay the same. Duplicates of the updated part, such as the type error in the example above, may change. \todo{say something about delta in precondition, plus the fact that it is on equivalence class?}
