...
Given $\present$, we will specify a total function $\interpret$, which maps a lower level back onto a higher level: \todo{Unclear: Say some things on totality of \interpret (is it easy to make it total?)??}
...
\begin{figure}
    \begin{center}   
      ...
      \caption{A single edit step.} \label{singleLayerEdit2}  % visio: Layer.vsd
    \end{center}
\end{figure}

after which the editor computes the final values for the data levels: $level''\L$ and $level''\H$, in order to restore the presentation invariant. Figure~\ref{singleLayerEditFormal} (which also shown in the previous chapter) illustrates the edit process.
...
Note that \law{Pres-Inert} is a special case of \law{Imprecise}, in which ``close~to'' is equality. In order to prevent a conflict with \law{Postcondition}, \law{Imprecise} is slightly weaker than \law{Postcondition}.
%\note{Maybe mention precondition more explicitly?} 
%\todo{say that \law{Imprecise} is weaker} is not true. 
...
Together with $Comp$, the requirements above provide a specification of $\interpret$. We can prove that \law{Postcondition} holds, and furthermore, if \law{InterPresent} holds, then \law{Doc-Inert} and \law{Pres-Inert} can be proved as well. \law{Imprecise} cannot be proved from \law{InterPresent}, and hence is a requirement on $\interpret$ (together with $Comp$).
...
%conclusions
% give present, we have two requirements interpresent, imprecise. and together with $comp$ these specify an editor.
...
Whitespace is an example of presentation extra state on the lower level. Consider the expression \verb|Sum 1 2|, which is presented as {\tt 1\spc+\spc 2}. The presentation is equivalent to a string containing `\verb|1|', `\verb|+|', and `\verb|2|' with any configuration of whitespace. Thus, for example, $(\texttt{1\spc+\spc 2})~L~(\texttt{1+2})$.
...
In order to disambiguate different $\reuz$ functions, we add the relation as a subscript to the operator (e.g.\ $\reuze{H}$ and $\reuze{L}\!$). For the higher level we have $\reuze{H} \tp  \Eqcl{Level\H}{H} \to Level\H \to Level\H$, and for the lower level $\reuze{L} \tp  \Eqcl{Level\L}{L} \to Level\L \to Level\L$
...
For the \law{Doc-Preserve} and \law{Imprecise} requirements, we give an informal argumentation because of the informal nature of the \close requirement. 
...
The wildcard for type $T$ stands for any possible value of type $T$. Thus, the value $\texttt{*}_{Bool}$ represents the set $\set{{\tt True}, {\tt False}}$, and the value $\texttt{*}_{Int}$ represents the set of all integers. An example shows how wildcards can be used to model extra state in a data type.
...
For a type $T$, a type $T^*$ declares which parts are extra state. Note that for a single type $T$ several possible types $T^*$ exist: whether or not a value is extra state is not determined by its type, but rather by the $T^*$ declarations in which  the value appears as a child. Hence, in the example, a boolean in a ${\tt Bin}$ node is extra state, whereas a boolean in a ${\tt Leaf}$ is not. 
...
\xpr{
\eqcl{x}{R} = \eqcl{x'}{R}  ~~\imp~~ \eqcl{x}{R} \reuz y \equ \eqcl{x'}{R} \reuz y
}
\begin{proof}
\begin{Prf}&
...
In order to describe the architecture of Proxima, we need to refine the single layer model of the previous sections to accommodate multiple layers. Given a function $\present$ that can be split into component functions $\present\H$ and $\present\L$, we show how the specification for $\interpret$ can be split into specifications for the components $\interpret\H$ and $\interpret\L$.  

\bc Rather than splitting aconsidering all layers at once, we look at a combined layer that consists of an upper and a lower layer with a middle data level in between. \ec

\todo{what about es?? partially specified?}
\bc Each layer of the composition may have presentation as well as interpretation extra state, which together determine the extra state for the combination. \ec

Instead of splitting a layer into $n$ components, we split it into an upper and a lower layer with a middle level in between. The upper layer may be a combined layer itself, whereas the lower layer is a single layer, such as specified in the previous section. This difference does not become important until Section~\ref{sect:defCombPres}, which also explains why the upper rather than the lower layer may be a combined layer.

It turns out that a layer cannot be split into arbitrary component layers. If we split the presentation mapping and construct $\interpret\H$ and $\interpret\L$ according to the specification, the result is not always a valid specification. Hence, we provide extra requirements on the components, to ensure that the specification is valid.

\bc We provide a condition that needs to hold for the components of the presentation mapping. Furthermore, even when the presentation mapping is split, and it turns out that the condition that the upper and lower layer both meet the requirements, is not sufficient to guarantee that the combination also meets the requirements. \ec
...
\subsection{Splitting $Present$ and $Interpret$} \label{sect:combinedExtraCompose}

Given a function $\present\C$:

\xpr{
\present\C &\tp& \Eqcl{\Level\H}{CH} \rightarrow  \Eqcl{\Level\L}{CL}\\
}

we specify the function $\interpret\C$:
...
We split the presentation and interpretation relations both into two component relations. When regarded as relations, we can split $present\C$ and $interpret\C$ as follows:
...
Before we show how to split $\present\C$ and $\interpret\C$ can be split as functions between equivalence classes, we first take a look at the equivalence classes that are involved.

When regarded as functions between equivalence classes, the two components of  $\present\C$:
...
%First, we take a look at the equivalence relations that play a role in the combination of two layers. 
Both the upper and lower layer have equivalence relations on their respective upper and lower levels: $HH$, $HL$, $LH$, and $LL$. 
...
($HL \neq LH$), because the presentation extra state of the upper layer is most likely different from the interpretation extra state of the lower layer.  Furthermore, on $Level\H$ and $\Level\L$, it is likely that the equivalence relations for $\present\C$ ($CH$ and $CL$) are not equal to the relations of the two composing layers ($HH$ and $LL$). Therefore, in total, we have the six equivalence relations that are shown in Figure~\ref{relationsInLayer}.
...
A simple example shows that equivalence relations on one level can be different. Consider a function $\present\C$  with its type written in the wildcard style from Section~\ref{sect:wildcardEq}.

\xpr{
\present\C \tp (Int, *_{Int}) \to (Int,Int)\\
\present\C (x,y) = (x)\\
}

Because $\present\C = \id \oo \present\C$, we can split $\present\C$ as follows:
...
It turns out that when we split $\present\C$ into $\present\H$ and $\present\L$, and construct $\interpret\H$ and $\interpret\L$ that meet the requirements of the previous section, this does not always lead to a correct specification of $\interpret\C$. A simple example shows this fact. Consider 
$\present\C = \id_{Int}$, which is split into the $\present\H$ and $\present\L$ below. The provided $\interpret\H$ and $\interpret\L$ meet the requirements of the previous section. 
...
%By taking $(0,0)$ and $(1,0)$ for the middle level $m$, we have: $\eqcl{0}{CH} = \interpret\C~\eqcl{0}{CL}$ and $\eqcl{1}{CH} %= \interpret\C~\eqcl{0}{CL}$, which implies $\eqcl{0}{CH} = \eqcl{1}{CH}$. 
%(take $m = (0,0)$) we must also have $\eqcl{0}{CL} = \present\C~\eqcl{1}{CH}$. However, the latter is clearly not the case, %since the only higher-layer presentation of $1$ is $(1,1)$, which has lower-layer presentation $1$, and not $0$.

%We have $\eqcl{0}{CL} = \present\C~\eqcl{0}{CH}$, and which holds for the components $\present\H$ and $\present\L$ (take the %middle level to be $(0,0)$). 

If we take $(0,0)$ for $m$ and use \law{$\interpret\C$-Def}, we have $\interpret\C \eqcl{0}{CL} = \eqcl{0}{CH}$. Similarly, for $m=(1,0)$, we get $\interpret\C \eqcl{0}{CL} = \eqcl{1}{CH}$, and hence, $\eqcl{0}{CH} \eq \eqcl{1}{CH}$. However, because $\present\C$ is the identity function, $CH$ is the equality relation, which implies $\eqcl{0}{CH} \neq \eqcl{1}{CH}$.

More generally, \law{$\present\C$-Def} puts a restriction on $\present\H$ and $\present\L$, which is that the equivalence relations of the components are such that \law{$\present\C$-Def} is a valid definition. However, even if the combination of $\present\H$ and $\present\L$ is valid and corresponds to $\present\C$, not every pair of $\interpret\H$ and $\interpret\L$ functions that meet the specification of the previous section, will lead to a valid definition of $\interpret\C$.

\bc
specify a correct editor.cannot be split into just any two components, even though  composition of two mappings between equivalence classes is itself not always a mapping between equivalence classes, in which case the two definitions are not valid.\ec
...
Without showing the details of their origin, we introduce two explicit conditions that need to hold in order for \law{$\present\C$-Def} and \law{$\interpret\C$-Def} to be valid. First, we define an auxiliary function $\mathcal{I}$. The application $\mathcal{I}~m$ denotes the set of all higher-level elements onto which the members of the $LH$ equivalence class of $m$ can be interpreted.

%in order correctly guarantee that the relations induce equivalence classes as well as that these equivalence classes are equal.
...
The \law{InterPresent} requirement holds for a combination if it holds for the upper and lower layer.
...
In this section, we give inductive versions of \law{$\present\C$-Def} and \law{$\interpret\C$-Def} for an editor consisting of $n$ layers. The building blocks of the combination are $n+1$ data levels and $n$ pairs of presentation and interpretation functions and corresponding equivalence relations:
...
After the lower-level update, the lower layer computes an intermediate value $\level'\M$, from which the higher layer (which may be a combined layer itself) computes $\level'\H$. At the top, $\level'\H$ is assigned to $\level''\H$, which is subsequently presented onto $level''\M$. Finally, $level''\M$ presented onto $level''\L$. 

Figure~\ref{combinedEdit} sketches the updates to the various data levels. Note that the figure is just a sketch, since $\present$ and $\interpret$ are functions between equivalence classes instead of values (as the figure might suggest). 
...
\end{Prf}
\todo{Maybe reverse these proofs after all?}
\end{proof}
...
Similar to \law{Doc-Inert}, we cannot guarantee \law{Pres-Inert} without assuming extra conditions. It turns out that we can prove \law{Pres-Inert} if we assume \law{$\present$-Match} \todo{not \law{$\interpret$-Match}??} together with a reversed \law{Orthogonal}, in which the $HL$ and $LH$ relations are swapped. However, because \law{Orthogonal} refers to $\reuze{LH}$, its reverse would have to refer to $\reuze{HL}$, which does not exist. The reason for this is because of the association order in $Comp$, reuse in the presentation direction of the higher layer is the result of the combined effect of the $\reuze{LL}$ functions of the components of the higher layer. Thus, the only way to specify a reversed \law{Orthogonal} is as a condition on the combined layer:
...
To allow interpretation extra state at the top-most combined layer, we give a separate proof the case $n=1$ for \law{Pres-Inert}, in which we do not use \law{Absorption}. 

An important consequence of the fact that we do not assume \law{Absorption} on the top-most combined layer is that in the proof of \law{Doc-Inert}, we cannot use \law{Absorption} to imply \law{$\present$-Match}, \law{$\interpret$-Match}, and \law{Orthogonal} for case $n=1$. However, since for $n=1$, all three conditions trivially hold, we do not need to assume \law{Absorption} for this case.
...
{\bf Case} $n=0$: We omit the trivial proof of this case. \todo{mention we don't need it because we give $n=1$? (and since $n=0$ is only for \id-layer)}
...
\Eqv{\law{InterPresent }}&
	\level'\L = \present~ \eqcl{\interpret~(\present~\eqcl{h}{LH})  ~\reuze{LH}~  \level\M}{LH}   ~\reuze{LL}~   
\level'\L\\
\Eqv{assumption}&
	\level'\L = \present~ \eqcl{\interpret~ \eqcl{\level'\L}{LL}  ~\reuze{LH}~   \level\M}{LH}   ~\reuze{LL}~   \level'\L\\
\Eqv{\law{\fn{wp}-$\gts$}}&
	\wp{\level'\M \gets \interpret~ \eqcl{\level'\L}{LL}  ~\reuze{LH}~   \level\M}{\level'\L = \present~ \eqcl{\level'\M}{LH}   ~\reuze{LL}~   \level'\L }\\
\Eqv{\law{$Up$-Def}}&
...
This completes proof of the first conjunct of the postcondition. For the second conjunct, we need to prove:

\xpr{ \eqcl{\level'\L}{LH} = \present\L~\eqcl{h}{LH}  \land \level'_{2} = x ~\imp\\ \wp{Comp_1}{\level'_{2} = x}}

which trivially holds, because $Comp_1$ only does assignments on levels 0 and 1. Hence, by \law{\fn{wp}-And}, we can conclude: 
...
\todo{more on nature of combined closeness?}
%\bl
...
\subsection{Conclusions}
\todo{make explicit that we only need \law{InterPresent} and \law{Imprecise}?}

A layered editor consisting of two component layers can be specified in the same manner as a single layered editor, but imposes additional requirements on the components. In order to guarantee that the combination is valid, \bc mapping between equivalence classes, \ec we need the 
...
\toHere
Het is allemaal zo erg niet. Formeel lastig, maar eigenlijk wel duidelijk (misschien even uitleggen hoe). Daarom volgende secties vooral schets
\fromHere
...
We want to express $R \tp X \rel Y$ as a bijective function $f \tp \Eqcl{Y}{H} \to \Eqcl{X}{L}$.
...
We want:$x~R~y \eqv \eqcl{x}{L} = f~\eqcl{y}{H}$.
