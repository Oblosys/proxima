...
The \p{Decl} constructor has three tokens: a structural token for the type signature, and two tokens for  ``\p{=}'' and ``\p{;}'' tokens. The other declarations (\p{BoardDecl} and \p{SlidesDecl}) only have a single token (for the keywords ``\p{Chess}'' and ``\p{Slides}'').\nonote{even uitleggen dat structural token niet opgeslagen wordt hier. Sowieso even beter bekijken} In the \p{Exp} type, we see one token for a \p{PlusExp}, which is for the ``\p{+}'' operator, and one token for a \p{DivExp}, which is a structural token for the entire fraction ``\p{$\frac{e_1}{e_2}$}''. The \p{LambdaExp} has two tokens (``$\lambda$'' and ``$\rarr\!\!$''), and the \p{BoolExp} and \p{IntExp} both have one.  The rest of the types have structural presentations and thus contain no tokens. 
...
The \p{SEM} keyword denotes the start of an attribution rule (also called a semantic function). The presentation is a synthesized attribute \p{pres}, which is denoted by \p{lhs.pres}. \nonote{iets meer AG syntax} The fraction is presented as a \p{parsing} presentation consisting of a single structural token.
