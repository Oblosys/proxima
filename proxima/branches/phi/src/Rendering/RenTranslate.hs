...
interpret state renLvl@(RenderingLevel scale c hp r fr rh frh sz debugging ur lmd)
...
-- TODO: make selectors scaleR and debuggingR for RenderingLevel
    ...
    KeySpecialRen UpKey   (Modifiers False False True) -> (SkipArr 0, state, RenderingLevel (scale*2) c hp r fr rh frh sz debugging ur lmd)
    KeySpecialRen DownKey (Modifiers False False True) -> (SkipArr 0, state, RenderingLevel (scale/2) c hp r fr rh frh sz debugging ur lmd)
    KeySpecialRen F9Key ms                             -> (SkipArr 0, state, RenderingLevel scale c hp r fr rh frh sz (not debugging) ur lmd)
    ...
    MouseDownRen x y ms i -> (MouseDownArr (descaleInt scale x) (descaleInt scale y) ms i, state, RenderingLevel scale c hp r fr rh frh sz debugging ur True)
    MouseDragRen x y ms   -> (MouseDragArr (descaleInt scale x) (descaleInt scale y) ms, state, renLvl)
    MouseUpRen x y ms     -> (MouseUpArr (descaleInt scale x) (descaleInt scale y) ms, state, RenderingLevel scale c hp r fr rh frh sz debugging ur False)
