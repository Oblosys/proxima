...
pathAFromPathP' []       _                      = []
pathAFromPathP' (p:path) (RowP id rf press)     = p:pathAFromPathP' path (index "ArrLayerUtils.pathAFromPathP'" press p)
pathAFromPathP' (p:path) (ColP id rf press)     = p:pathAFromPathP' path (index "ArrLayerUtils.pathAFromPathP'" press p)                                            
pathAFromPathP' (p:path) (OverlayP id press)    = p:pathAFromPathP' path (index "ArrLayerUtils.pathAFromPathP'" press p)                                            
pathAFromPathP' (p:path) (GraphP _ _ _ _ _ press) = p:pathAFromPathP' path (index "ArrLayerUtils.pathAFromPathP'" press p) -- edges are put after vertices, so selection is ok
...
pathPFromPathA' []       _                          = []
pathPFromPathA' (p:path) (RowP id rf press)         = p:pathPFromPathA' path (index "ArrLayerUtils.pathPFromPathA'" press p)
pathPFromPathA' (p:path) (ColP id rf press)         = p:pathPFromPathA' path (index "ArrLayerUtils.pathPFromPathA'" press p)                                            
pathPFromPathA' (p:path) (GraphP _ _ _ _ _ press)   = if p >= length press then [p] -- selection is on edge
                                                      else p:pathPFromPathA' path (index "ArrLayerUtils.pathPFromPathA'" press p)                                            
