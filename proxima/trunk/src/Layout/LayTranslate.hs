...
translateIO :: (Show token, DocNode node) => ScannerSheet doc node clip token -> LayerStateLay doc node clip token -> LayoutLevel doc node clip token -> PresentationLevel doc node clip token -> EditLayout documentLevel doc node clip token
            -> IO (EditPresentation documentLevel doc node clip token, LayerStateLay doc node clip token, LayoutLevel doc node clip token)
...
parseIO :: (Show token, DocNode node) => ScannerSheet doc node clip token -> LayerStateLay doc node clip token -> LayoutLevel doc node clip token -> PresentationLevel doc node clip token -> EditLayout documentLevel doc node clip token -> IO (EditPresentation documentLevel doc node clip token, LayerStateLay doc node clip token, LayoutLevel doc node clip token)
...
--editLay :: 
--            Layout doc node clip token -> Layout doc node clip token -> LayoutLevel doc node clip token -> FocusPres -> (EditPresentation documentLevel doc node clip token, Layout doc node clip token, Layout doc node clip token)
...
openFile :: DocNode node => String -> Layout doc node clip token -> LayoutLevel doc node clip token -> PresentationLevel doc node clip token -> IO (EditPresentation documentLevel doc node clip token, Layout doc node clip token, LayoutLevel doc node clip token)
...
editInsert :: DocNode node => Char -> Layout doc node clip token -> LayoutLevel doc node clip token -> (LayoutLevel doc node clip token, Layout doc node clip token)
...
editCut :: (DocNode node, Show token) => Layout doc node clip token -> LayoutLevel doc node clip token -> (LayoutLevel doc node clip token, Layout doc node clip token)
editCut clip (LayoutLevel pres focus dt) = 
  let clip' = copyTree focus clip pres                                                                                              
      (pres', focus') = deleteTree focus pres
  in  (LayoutLevel pres' focus' dt, clip')

editCopy :: (DocNode node, Show token) => Layout doc node clip token -> LayoutLevel doc node clip token -> PresentationLevel doc node clip token -> (EditPresentation documentLevel doc node clip token, Layout doc node clip token, LayoutLevel doc node clip token)
...
editNormalize :: (DocNode node, Show token) => Layout doc node clip token -> LayoutLevel doc node clip token -> (LayoutLevel doc node clip token, Layout doc node clip token)
...
navigateLeft :: (DocNode node, Show token) => Layout doc node clip token -> LayoutLevel doc node clip token -> PresentationLevel doc node clip token -> (EditPresentation documentLevel doc node clip token, Layout doc node clip token, LayoutLevel doc node clip token)
...
navigateRight :: (DocNode node, Show token) => Layout doc node clip token -> LayoutLevel doc node clip token -> PresentationLevel doc node clip token -> (EditPresentation documentLevel doc node clip token, Layout doc node clip token, LayoutLevel doc node clip token)
...
enlargeLeft :: (DocNode node, Show token) => Layout doc node clip token -> LayoutLevel doc node clip token -> PresentationLevel doc node clip token -> (EditPresentation documentLevel doc node clip token, Layout doc node clip token, LayoutLevel doc node clip token)
...
enlargeRight :: (DocNode node, Show token) => Layout doc node clip token -> LayoutLevel doc node clip token -> PresentationLevel doc node clip token -> (EditPresentation documentLevel doc node clip token, Layout doc node clip token, LayoutLevel doc node clip token)
...
graphEdit :: (Show token, DocNode node) => ScannerSheet doc node clip token ->
             LayerStateLay doc node clip token -> LayoutLevel doc node clip token ->
             PresentationLevel doc node clip token ->
             (LayerStateLay doc node clip token -> LayoutLevel doc node clip token -> (LayerStateLay doc node clip token, LayoutLevel doc node clip token))->
             (EditPresentation documentLevel doc node clip token, LayerStateLay doc node clip token, LayoutLevel doc node clip token) 
...
addVertex :: DocNode node => [Int] -> (Int, Int) ->  LayerStateLay doc node clip token -> LayoutLevel doc node clip token ->
             (LayerStateLay doc node clip token, LayoutLevel doc node clip token)
...
{-
openFile :: Layout doc node clip -> LayoutLevel doc node clip token -> PresentationLevel doc node clip token -> FilePath -> IO (EditPresentation documentLevel doc node clip token, Layout doc node clip, LayoutLevel doc node clip token) 
...
saveFile :: Layout doc node clip -> LayoutLevel doc node clip token -> PresentationLevel doc node clip token -> FilePath -> IO (EditPresentation documentLevel doc node clip token, Layout doc node clip, LayoutLevel doc node clip token)
