...
instance ReductionSheet Document EnrichedDoc ClipDoc where
  ...
  reductionSheetSimple state (RootEnr (RootE _   _    oldIdlDcls) _) _ 
                      enrDoc@(RootEnr (RootE idp dcls idldcls)    _) =
  ...
  reductionSheetSimple state _ _ enrDoc =
    case enrDoc of 
      ...
      (RootEnr (ParseErrRootE p e ts) _) ->
        (RootDoc (ParseErrRoot p e ts), state, enrDoc )
      (RootEnr HoleRootE _) ->
        (RootDoc HoleRoot, state, enrDoc )
      HoleEnrichedDoc ->
        (HoleDocument,state, enrDoc )
      ParseErrEnrichedDoc prs e ts ->
        (ParseErrDocument prs e ts, state, enrDoc )
...
instance Eq List_Decl where
  ...
  (ParseErrList_Decl _ _ _) == _                     = True
  _                     == (ParseErrList_Decl _ _ _) = True
  _                     == _                     = False
...
instance Eq Decl where
  ...
  (ParseErrDecl _ _ _)          == _                           = True
  _                           == (ParseErrDecl _ _ _)            = True
  _                           == _                           = False        

instance Eq Ident where
  (Ident _ _ str1)  == (Ident _ _ str2)  = str1 == str2
  HoleIdent         == HoleIdent         = True
  (ParseErrIdent _ _ _) == _                 = True
  _                 == (ParseErrIdent _ _ _) = True
  _                 == _                 = False        
