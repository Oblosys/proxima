...
initialDocument = 
 do { let filePath = "Heliumfile.hs"
    ...
    ; return $ RootDoc $ Root NoIDP (ParseErrList_Decl (ColP NoIDP 0 NF . map (StringP NoIDP). lines' $ fileContents) (0,"") [])                                   
    }
...
toXMLDocument (ParseErrDocument presentation error tokens) = Elt "ParseErrDocument" [] []
toXMLRoot (Root _ decls) = Elt "Root" [] $ toXMLList_Decl decls
toXMLRoot (HoleRoot) = Elt "HoleRoot" [] $ []
toXMLRoot (ParseErrRoot presentation error tokens) = Elt "ParseErrRoot" [] []
...
toXMLEnrichedDoc (ParseErrEnrichedDoc presentation error tokens) = Elt "ParseErrEnrichedDoc" [] []
...
toXMLRootE (ParseErrRootE presentation error tokens) = Elt "ParseErrRootE" [] []
...
toXMLDecl (ParseErrDecl presentation error tokens) = Elt "ParseErrDecl" [] []
toXMLIdent (Ident _ _ string) = Elt "Ident" [] $ [toXMLString string]
toXMLIdent (HoleIdent) = Elt "HoleIdent" [] $ []
toXMLIdent (ParseErrIdent presentation error tokens) = Elt "ParseErrIdent" [] []
...
toXMLExp (ParseErrExp presentation error tokens) = Elt "ParseErrExp" [] []
toXMLAlt (Alt _ _ ident exp) = Elt "Alt" [] $ [toXMLIdent ident] ++ [toXMLExp exp]
toXMLAlt (HoleAlt) = Elt "HoleAlt" [] $ []
toXMLAlt (ParseErrAlt presentation error tokens) = Elt "ParseErrAlt" [] []
...
toXMLBoard (ParseErrBoard presentation error tokens) = Elt "ParseErrBoard" [] []
...
toXMLBoardRow (ParseErrBoardRow presentation error tokens) = Elt "ParseErrBoardRow" [] []
...
toXMLBoardSquare (ParseErrBoardSquare presentation error tokens) = Elt "ParseErrBoardSquare" [] []
...
toXMLPPPresentation (ParseErrPPPresentation presentation error tokens) = Elt "ParseErrPPPresentation" [] []
toXMLSlide (Slide title itemList) = Elt "Slide" [] $ [toXMLString title] ++ [toXMLItemList itemList]
toXMLSlide (HoleSlide) = Elt "HoleSlide" [] $ []
toXMLSlide (ParseErrSlide presentation error tokens) = Elt "ParseErrSlide" [] []
...
toXMLItemList (ParseErrItemList presentation error tokens) = Elt "ParseErrItemList" [] []
...
toXMLListType (ParseErrListType presentation error tokens) = Elt "ParseErrListType" [] []
