...
genAnonymousPattern (Prod _ cnstrName idpFields fields) = "("++ cnstrName++ concat(replicate (length idpFields) " _")  ++ prefixBy " x" (map show [1..length fields]) ++ ")"
genPattern' (Prod _ cnstrName idpFields fields) = 
  "("++ cnstrName ++prefixBy " i" (map show [1..length idpFields]) ++ prefixBy " x" (map show [1..length fields]) ++ ")"

genEditable decls = genBanner "Editable instances" $
  concatMap genEditableDecl decls

-- TODO remove -1 from idents
-- remove getAnonymousPattern
genEditableDecl (Decl (LHSBasicType typeName) prods) = 
  ...
  , ""
  , "  paste [] (Clip_%1 c) _      = c"
  , "  paste [] c  x                    = trace (\"Type error: pasting \"++show c++\" on %1\")   x"
  ] <~ [typeName] ++
  ...
  where genSelect prod@(Prod _ cnstrName idpFields fields) = 
          [ "  select (%1:p) %2 = select p x%3"
            <~ [ show i, genAnonymousPattern prod, show $ i+1 ] 
          | i <- [ 0..length fields -1 ]]
        genPaste prod@(Prod _ cnstrName idpFields fields) =
          [ "  paste (%1:p) c %2 = %3"
            <~ [ show i, genPattern' prod
               , cnstrName ++ prefixBy " i" (map show [1..length idpFields]) ++ concat
                   [ if j-1 == i then " (paste p c x" ++ show j ++ ")" else " x"++show j | j <- [1..length fields ] ]
               ] 
          | i <- [ 0..length fields -1 ]]
        ...
        genArity prod@(Prod _ cnstrName idpFields fields) = 
          "  arity %1 = %2" <~ [ genAnonymousPattern prod, show $ length fields ]
...
genEditableDecl (Decl (LHSListType typeName) prods) = 
  ...
  , "  select (n:p) (List_%1 cxs) = let xs = fromConsList_%1 cxs"
  , "                                  in  if n < length xs "
  , "                                      then select p (xs !! n)"
  , "                                      else Clip_Nothing"
  , "  select _     _                  = Clip_Nothing"
  ...
  , "  paste [] c  x                  = trace (\"Type error: pasting \"++show c++\" on List_%1\")   x"
  , "  paste (n:p) c (List_%1 cxs) = let xs = fromConsList_%1 cxs"
  , "                                    in  if n < length xs"
  , "                                        then let x  = xs!!n"
  , "                                                 x' = paste p c x"
  ...
  , "  insertList _ _             xs = trace \"Type error, no paste\" $ Clip_List_%1 xs"
